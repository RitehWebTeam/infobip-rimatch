...
export const ProfileCard: React.FC<ProfileCardProps> = ({ user, onClose }) => {
  return (
    <div className="flex justify-center items-center overflow-y-hidden">
      <div className="relative flex flex-col items-center  rounded-[25px] border-[1px] border-black-200 h-[900px] sm:h-[700px] w-[550px] p-4 dark:bg-[#343030] bg-clip-border border-[#acabab33] shadow-xl shadow-black">
        ...
        <div className="flex flex-col  items-center w-full ">
          ...
          <div className="fixed bottom-16 w-full flex flex-col items-center">
            <p className="mt-4 border-t border-gray-500  text-center">TAGS</p>
            <div className="mt-1">{user.tags.map((tag) => `#${tag}`)}</div>
          </div>

          <button
            className=" fixed bottom-0 mb-7  text-gray-500 hover:text-gray-300"
            onClick={onClose}
          >
            Close Profile
          </button>
